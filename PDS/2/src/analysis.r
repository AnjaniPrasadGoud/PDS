...
Student = read.csv("C:\\Users\\anjan\\OneDrive\\Documents\\PDS\\2\\data_clean\\clean_StudentsData.csv")
Student
...
plot(Student_Data$gender, Student_Data$reading.score,
     xlab = "SEX", ylab = "READING SCORE",main="BOXPLOTS OF READING SCORE VS GENDER",col.main="skyblue", col=(c("orange","darkgreen")))

#2.3
Student_Data$gender <- as.factor(Student_Data$gender)
plot(Student_Data$gender, Student_Data$writing.score,
     xlab = "SEX", ylab = "WRITING SCORE",main="BOXPLOTS OF WRITING SCORE VS GENDER",col.main="purple", col=(c("navyblue","red")))
...
ggplot(data = Student_Data, aes(x = math.score, y = reading.score, col = gender)) +
  ...
  scale_color_manual(values = c("male" = "green", "female" = "gold")) +
...
hist(Student_Data$reading.score,xlim=c(0,100), col=9, xlab = "READING SCORE", main = "Distribution Reading Score")
